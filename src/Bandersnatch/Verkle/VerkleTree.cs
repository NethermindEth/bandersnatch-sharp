...
public class VerkleTree
{
    ...
    {
        // replace this node.key by node.StemNode
        (List<byte> sharedPath, byte? pathDiffIndexOld, byte? pathDiffIndexNew) =
            VerkleUtils.GetPathDifference(node.Stem, traverseContext.Key[..31].ToArray());
        ...
        _db.StemTable.TryGetValue(traverseContext.Key[..31].ToArray(), out SuffixTree oldValue);
    ...
    private (Fr, Commitment?) UpdateSuffixNode(byte[] stemKey, LeafUpdateDelta leafUpdateDelta, byte suffixLeafIndex, bool insertNew = false)
    {
        SuffixTree oldNode;
