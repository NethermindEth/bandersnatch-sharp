...
namespace Nethermind.Tree.Forest
{
    public class TreeDumper : ITreeVisitor
    {
        ...
        {
            _builder.AppendLine($"{GetPrefix(trieVisitContext)}BRANCH | -> {(node.Commitment?.Bytes ?? node.FullRlp)?.ToHexString()}");
        }

        public void VisitExtension(TrieNode node, TrieVisitContext trieVisitContext)
        {
            _builder.AppendLine($"{GetPrefix(trieVisitContext)}EXTENSION {Nibbles.FromBytes(node.Path).ToPackedByteArray().ToHexString(false)} -> {(node.Commitment?.Bytes ?? node.FullRlp)?.ToHexString()}");
        }
        ...
        public void VisitLeaf(TrieNode node, TrieVisitContext trieVisitContext, byte[] value = null)
        {
            ...
            _builder.AppendLine($"{GetPrefix(trieVisitContext)}{leafDescription} {Nibbles.FromBytes(node.Path).ToPackedByteArray().ToHexString(false)} -> {(node.Commitment?.Bytes ?? node.FullRlp)?.ToHexString()}");
