...
public class TrieStore : ITrieStore
{
    private class DirtyNodesCache
    {
        ...
        public ConcurrentDictionary<Commitment, TrieNode> AllNodes => _objectsCache;

        private readonly ConcurrentDictionary<Commitment, TrieNode> _objectsCache = new();
        ...
        public void Dump()
        {
            if (_trieStore._logger.IsTrace)
            {
                ...
                foreach (KeyValuePair<Commitment, TrieNode> keyValuePair in _objectsCache)
                {
    ...
    internal byte[] LoadRlp(Commitment Commitment, IKeyValueStore? keyValueStore)
    {
        keyValueStore ??= _keyValueStore;
        byte[]? rlp = _currentBatch?[Commitment.Bytes] ?? keyValueStore[Commitment.Bytes];

        if (rlp is null)
        {
            throw new TrieException($"Node {Commitment} is missing from the DB");
        }
    ...
    public byte[] LoadRlp(Commitment Commitment) => LoadRlp(Commitment, null);

    public bool IsPersisted(Commitment Commitment)
    {
    ...
    private void PruneCache()
    {
        ...
        long newMemory = 0;
        foreach ((Commitment key, TrieNode node) in _dirtyNodes.AllNodes)
        {
            if (node.IsPersisted)
            {
                ...
                if (node.Commitment is null)
                {
                    ...
                    if (node.Commitment != key)
                    {
                        throw new InvalidOperationException($"Persisted {node} {key} != {node.Commitment}");
                    }
                }
        ...
        {
            if (trieNode.Commitment is null)
            {
                throw new InvalidOperationException($"{trieNode} has a null key");
            }

            _dirtyNodes.Remove(trieNode.Commitment!);
        }
    ...
    private void Persist(TrieNode currentNode, long blockNumber)
    {
        ...
        if (currentNode.Commitment is not null)
        {
            ...
            _currentBatch[currentNode.Commitment.Bytes] = currentNode.FullRlp;
        ...
        else
        {
            Debug.Assert(currentNode.FullRlp is not null && currentNode.FullRlp.Length < 32,
                "We only expect persistence call without Commitment for the nodes that are kept inside the parent RLP (less than 32 bytes).");
        }
    }
    ...
    public void PersistCache(IKeyValueStore store, CancellationToken cancellationToken)
    {
        Task.Run(() =>
        {
            ...
            KeyValuePair<Commitment, TrieNode>[] nodesCopy = _dirtyNodes.AllNodes.ToArray();
    ...
    public byte[]? this[byte[] key]
    {
        get => _pruningStrategy.PruningEnabled
               && _dirtyNodes.AllNodes.TryGetValue(new Commitment(key), out TrieNode? trieNode)
