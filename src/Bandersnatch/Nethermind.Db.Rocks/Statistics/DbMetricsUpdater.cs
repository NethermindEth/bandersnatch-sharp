...
namespace Nethermind.Db.Rocks.Statistics
{
    public class DbMetricsUpdater
    {
        ...
        {
            double offsetInSec = _dbConfig.StatsDumpPeriodSec * 1.1;
        ...
        private void UpdateMetrics(object? state)
        {
            try
            {
                ...
                string? compactionStatsString = _db.GetProperty("rocksdb.stats");
                ProcessCompactionStats(compactionStatsString);

                if (_dbConfig.EnableDbStatistics)
                {
                    string? dbStatsString = _dbOptions.GetStatisticsString();
        ...
        public void ProcessCompactionStats(string compactionStatsString)
        {
            ...
            {
                List<(string Name, long Value)> stats = ExctractStatsPerLevel(compactionStatsString);
        ...
        private void UpdateMetricsFromList(List<(string Name, long Value)> levelStats)
        {
            ...
            {
                foreach ((string Name, long Value) stat in levelStats)
                {
        ...
        /// <summary>
        ///     Example line:
        ///     L0      2/0    1.77 MB   0.5      0.0     0.0      0.0       0.4      0.4       0.0   1.0      0.0     44.6
        ///     9.83              0.00       386    0.025       0      0
        /// </summary>
        private List<(string Name, long Value)> ExctractStatsPerLevel(string compactionStatsDump)
        {
            List<(string Name, long Value)> stats = new List<(string Name, long Value)>(5);

            if (!string.IsNullOrEmpty(compactionStatsDump))
            {
                Regex rgx = new Regex(@"^\s+L(\d+)\s+(\d+)\/(\d+).*$", RegexOptions.Multiline);
        ...
        private List<(string Name, long Value)> ExctractIntervalCompaction(string compactionStatsDump)
        {
            ...
            if (!string.IsNullOrEmpty(compactionStatsDump))
            {
                Regex rgx = new Regex(@"^Interval compaction: (\d+)\.\d+.*GB write.*\s+(\d+)\.\d+.*MB\/s write.*\s+(\d+)\.\d+.*GB read.*\s+(\d+)\.\d+.*MB\/s read.*\s+(\d+)\.\d+.*seconds.*$", RegexOptions.Multiline);
