...
namespace Nethermind.Verkle.Tree.VerkleStateDb
{
    public class DiskDb : IVerkleDb
    {
        ...
        {
            SetLeaf(leafKey, leafValue, LeafDb);
        }
        ...
        {
            SetBranch(branchKey, internalNodeValue, BranchDb);
        }
        ...
        public void SetLeaf(byte[] leafKey, byte[]? leafValue, IKeyValueStore db)
        {
            db[leafKey] = leafValue;
        }

        public void SetStem(byte[] stemKey, SuffixTree? suffixTree, IKeyValueStore db)
        {
            db[stemKey] = SuffixTreeSerializer.Instance.Encode(suffixTree).Bytes;
        }

        public void SetBranch(byte[] branchKey, InternalNode? internalNodeValue, IKeyValueStore db)
        {
            db[branchKey] = InternalNodeSerializer.Instance.Encode(internalNodeValue).Bytes;
        }
    }
}
