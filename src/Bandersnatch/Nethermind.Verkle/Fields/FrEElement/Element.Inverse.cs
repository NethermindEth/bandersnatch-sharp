...
namespace Nethermind.Verkle.Fields.FrEElement
{
    public readonly partial struct FrE
    {
        ...
        private static ulong MulWNonModular(in FE x, long y, out FE res)
        {
            ...
            ulong c = Math.BigMul(x[0], w, out z[0]);
            c = MAdd1(x[1], w, c, out z[1]);
            c = MAdd1(x[2], w, c, out z[2]);
            c = MAdd1(x[3], w, c, out z[3]);
        ...
        private static ulong NegL(in FE x, ulong xHi, out FE res)
        {
            ulong b = 0;
            ulong[] z = new ulong[4];
            SubtractWithBorrow(0, x[0], ref b, out z[0]);
            SubtractWithBorrow(0, x[1], ref b, out z[1]);
            SubtractWithBorrow(0, x[2], ref b, out z[2]);
            SubtractWithBorrow(0, x[3], ref b, out z[3]);
        ...
        private static ulong Approximate(in FE x, int nBits)
        {
            ...
            ulong mask1 = ~(ulong)(((1 << (hiWordBitsAvailable - hiWordBitsUsed)) - 1));
            ulong hi = (x[hiWordIndex] & mask1) << (64 - hiWordBitsAvailable);

            mask1 = ~(ulong)((1 << (approxLowBitsN + hiWordBitsUsed) - 1));
        ...
        private static ulong LinearCombNonModular(in FE x, long xC, in FE y, long yC, out FE res)
        {
            ...
            AddWithCarry(res[0], yTimes[0], ref carry, out z[0]);
            AddWithCarry(res[1], yTimes[1], ref carry, out z[1]);
            AddWithCarry(res[2], yTimes[2], ref carry, out z[2]);
            AddWithCarry(res[3], yTimes[3], ref carry, out z[3]);
            res = z;
