...
namespace Nethermind.Serialization.Rlp
{
    ...
    public class Rlp
    {
        public const int LengthOfCommitmentRlp = 33;
        ...
        public static Rlp Encode(Commitment? Commitment)
        {
            ...
            if (ReferenceEquals(Commitment, Commitment.EmptyTreeHash))
            {
                return OfEmptyTreeHash;
            }

            if (ReferenceEquals(Commitment, Commitment.OfAnEmptyString))
            {
                return OfEmptyStringHash;
            }

            byte[] result = new byte[LengthOfCommitmentRlp];
            result[0] = 160;
            Buffer.BlockCopy(Commitment.Bytes, 0, result, 1, 32);
            return new Rlp(result);
        }
        ...
        public static Rlp Encode(IList<Commitment> sequence)
        {
        ...
        public ref struct ValueDecoderContext
        {
            ...
            private class DecodeCommitmentRlpException : RlpException
            {
                ...
                public DecodeCommitmentRlpException(string message) : base(message)
                {
                }

                public DecodeCommitmentRlpException(in int prefix, in int position, in int dataLength) : this(string.Empty)
                {
                    _prefix = prefix;
                    _position = position;
            ...
            public Commitment? DecodeCommitment()
            {
                ...
                if (prefix != 128 + 32)
                {
                    throw new DecodeCommitmentRlpException(prefix, Position, Data.Length);
                }
            ...
            public void DecodeCommitmentStructRef(out CommitmentStructRef commitment)
            {
                int prefix = ReadByte();
                if (prefix == 128)
                {
                ...
                else
                {
                    ...
                    else if (CommitmentSpan.SequenceEqual(Commitment.EmptyTreeHash.Bytes))
                    {
                        commitment = new CommitmentStructRef(Commitment.EmptyTreeHash.Bytes);
                    }

                    commitment = new CommitmentStructRef(CommitmentSpan);
                }
            }
            ...
            public Address? DecodeAddress()
            {
                ...
                if (prefix != 128 + 20)
                {
                    throw new RlpException($"Unexpected prefix of {prefix} when decoding {nameof(Commitment)} at position {Position} in the message of length {Data.Length} starting with {Data.Slice(0, Math.Min(DebugMessageContentLength, Data.Length)).ToHexString()}");
                }
