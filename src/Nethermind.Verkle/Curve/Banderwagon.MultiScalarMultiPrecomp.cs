...
public static class PrecompMSM2
{
    public static uint T { get; private set; }
    public static byte B { get; private set; }
    public static AffinePoint[] Table { get; private set; }
    public static int NumWindows { get; private set; }
    public static int BasisLen { get; private set; }
    ...
    static PrecompMSM2()
    {
        Banderwagon[] basisBanderwagon = Generate();
        AffinePoint[] basis = ConvertBanderwagonToAffinePoints(basisBanderwagon); 
        T = 5;
        B = 12;
        BasisLen = basis.Length;
        CreatePrecompMSM2(basis);
    }

    private static AffinePoint[] ConvertBanderwagonToAffinePoints(Banderwagon[] banderwagonArray)
    {
        AffinePoint[] affinePoints = new AffinePoint[banderwagonArray.Length];
        for (int i = 0; i < banderwagonArray.Length; i++)
        {
            affinePoints[i] = banderwagonArray[i].ToAffine();
        }
        return affinePoints;
    }

    private static void CreatePrecompMSM2(AffinePoint[] basis)
    {
        int windowSize = 1 << B;
        const int FrBits = 253;
        int pointsPerColumn = (FrBits + (int)T - 1) / (int)T;

        NumWindows = (pointsPerColumn * BasisLen + B - 1) / B;
        List<AffinePoint> tableBasis = new List<AffinePoint>(pointsPerColumn * BasisLen);

        foreach (var point in basis)
        {
            var currentPoint = point;
            tableBasis.Add(currentPoint);
            for (int i = 1; i < pointsPerColumn; i++)
            {
                for (int j = 0; j < T; j++)
                {
                    currentPoint = AffinePoint.Double(currentPoint);
                }
                tableBasis.Add(currentPoint);
            }
        }

        Table = new AffinePoint[windowSize * NumWindows];
        for (int w = 0; w < NumWindows; w++)
        {
            AffinePoint[] window = new AffinePoint[windowSize];

            int start = w * B;
            int end = (w + 1) * B;
            if (end > tableBasis.Count) end = tableBasis.Count;

            AffinePoint[] windowBasis = tableBasis.GetRange(start, end - start).ToArray();
            FillWindow(ref window, windowBasis); 
            Array.Copy(window, 0, Table, w * windowSize, window.Length);
        }

    }

    private static void FillWindow(ref AffinePoint[] table, AffinePoint[] basis)
    {
        int b = basis.Length;
        int windowSize = 1 << b; 
        ...
        {
            table[i] = AffinePoint.Identity;
        }
        
        for (int bit = 0; bit < b; bit++)
        {
            table[1 << bit] = basis[b - 1 - bit];
        }
        ...
        for (int i = 1; i < windowSize; i++)
        {
            for (int bit = 0; bit < b; bit++)
            {
                if ((i & (1 << bit)) != 0)
                {
                    table[i] = AffinePoint.Add(table[i], table[1 << bit]);
                }
            }
        }
    }
...
public readonly partial struct Banderwagon{
    public static (Banderwagon result, int addDoubleCount, Exception error) MSM(FrE[] montScalars)
    {
        if (montScalars.Length > PrecompMSM2.BasisLen)
        {
            return (default, 0, new Exception($"Scalars length ({montScalars.Length}) is bigger than basis length ({PrecompMSM2.BasisLen})."));
        }

        FrE[] scalars = new FrE[montScalars.Length];
        for (int i = 0; i < montScalars.Length; i++)
        {
            FrE.FromMontgomery(montScalars[i], out scalars[i]);
        }

        int addDoubleCount = 0;
        int windowSize = 1 << PrecompMSM2.B;
        AffinePoint accum = AffinePoint.Identity;

        for (uint t_i = 0; t_i < PrecompMSM2.T; t_i++)
        {
            if (t_i > 0)
            {
                accum = AffinePoint.Double(accum);
                addDoubleCount++;
            }
            ...
            int windowScalar = 0;
            byte windowBitPos = 0;
            for (int s_i = 0; s_i < scalars.Length; s_i++)
            {
                uint k = 0;
                while (k < 253)
                {
                    ...
                    windowBitPos++;
                    if (windowBitPos == PrecompMSM2.B)
                    {
                        if (windowScalar > 0)
                        {
                            var index = currWindow * windowSize + windowScalar;
                            var pointToAdd = PrecompMSM2.Table[index];
                            accum = AffinePoint.Add(accum, pointToAdd);
                            addDoubleCount++;
                        }
                        currWindow++;
                        windowScalar = 0;
        ...
        Banderwagon result = new Banderwagon(accum);
        return (result, addDoubleCount, null);
    }
}